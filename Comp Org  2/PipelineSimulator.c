...
typedef struct stateStruct {
  ...
  int BPB[NUMMEMORY];			  /* Branch Prediction Buffer */
...
void run(){
  ...
  initState(&state);         /* Initialize the state of the pipeline */
	...
	/* Modify newState stage-by-stage below to reflect the state of the pipeline after the cycle has executed */
        ...
        /* --------------------- IF stage --------------------- */
        //check if a stall due to LW is necessary
	  newState.IFID.instr = state.instrMem[(state.PC)/4];
	  newState.PC = state.PC + 4;
	  newState.IFID.PCPlus4 = state.PC + 4;
	
        /* --------------------- WB stage --------------------- */
	//write this code before the ID stage
	if(get_opcode(state.MEMWB.instr) == 35) //lw
	{
		newState.regFile[state.MEMWB.writeReg] = state.MEMWB.writeDataMem; 
	}
	else if((get_opcode(state.MEMWB.instr)==0) && (get_funct(state.MEMWB.instr)>=32))
	{
	...
	if(get_opcode(newState.IDEX.instr) == 35) //lw stall check
	{
		if(newState.IDEX.rtReg==get_rs(newState.IFID.instr))
		{
		...
		if((get_opcode(newState.IFID.instr) != 35) && (newState.IDEX.rtReg == get_rt(newState.IFID.instr)))
		{
			newState.IFID.instr = 0;
			newState.IFID.PCPlus4 = newState.PC;
			newState.PC = newState.PC - 4;
	...
	if(get_opcode(newState.IDEX.instr) == 4) //bne
	{
		newState.branches++;
		//check prediction buffer, stall and update pc according to result
		if(state.BPB[(newState.IDEX.PCPlus4-4)/4]>=2)  //branch taken
		{
			newState.IFID.instr = 0;
			newState.IFID.PCPlus4 = newState.IDEX.branchTarget;
        ...
        /* --------------------- EX stage --------------------- */
	//check if forwarding is needed in this stage
	newState.EXMEM.instr = state.IDEX.instr;
	if(get_opcode(newState.EXMEM.instr) == 0) //R type
	{
		if(get_funct(newState.EXMEM.instr) == 32) //add
		{
		  newState.EXMEM.aluResult = state.IDEX.readData1 + state.IDEX.readData2;
		}
		else if(get_funct(newState.EXMEM.instr) == 34) //sub
		{
		  newState.EXMEM.aluResult = state.IDEX.readData1 - state.IDEX.readData2;
		}
		...
		if((newState.EXMEM.writeReg == newState.IDEX.rtReg) && (get_opcode(newState.IDEX.instr) != 35))
		{
		  newState.IDEX.readData2 = newState.EXMEM.aluResult;
		}
		if(newState.EXMEM.writeReg == newState.IDEX.rsReg)
		{
		  newState.IDEX.readData1 = newState.EXMEM.aluResult;
		}
	}
	else if(get_opcode(newState.EXMEM.instr) == 35) //LW
	{
		newState.EXMEM.aluResult = state.IDEX.readData1 + state.IDEX.immed;
		newState.EXMEM.writeDataReg = state.IDEX.readData2;
		newState.EXMEM.writeReg = state.IDEX.rtReg;
		//forward checking for LW performed in MEM stage
	}
	else if(get_opcode(newState.EXMEM.instr) == 43) //SW
	{
		//SW never needs to forward
		newState.EXMEM.aluResult = state.IDEX.readData1 + state.IDEX.immed;
		newState.EXMEM.writeDataReg = state.IDEX.readData2;
		newState.EXMEM.writeReg = 0;
	}
	else if(get_opcode(newState.EXMEM.instr) == 4) //BNE
	{
		//BNE never needs to forward
		newState.EXMEM.aluResult = state.IDEX.readData1 - state.IDEX.readData2;
		if(newState.EXMEM.aluResult == 0)
		{
		  //branch should not have happened, check bpb and update registers accordingly
		  if(state.BPB[(state.IDEX.PCPlus4-4)/4]<2)  //branch not taken
		  {
			newState.BPB[(state.IDEX.PCPlus4-4)/4] = 0;
		  }
		  else if(state.BPB[(state.IDEX.PCPlus4-4)/4] >= 2) //branch taken, correct IDEX and IFID
		  {
			newState.BPB[(state.IDEX.PCPlus4 - 4)/4]--;
			newState.mispredicted++;
			newState.stalls = newState.stalls + 1;
		...
		{
		  //branch should have happened, check bpb and update registers accordingly
		  if(state.BPB[(state.IDEX.PCPlus4-4)/4]>=2)  //branch taken
		  {
			newState.BPB[(state.IDEX.PCPlus4-4)/4] = 3;
		  }
		  else if(state.BPB[(state.IDEX.PCPlus4 - 4)/4] < 2) //not taken, correct IDEX and IFID and PC
		  {
			newState.BPB[(state.IDEX.PCPlus4 - 4)/4]++;
			newState.mispredicted++;
			newState.stalls = newState.stalls + 2;
			newState.IDEX.instr = 0;
			newState.IFID.instr = 0;
	...
	if(get_opcode(newState.MEMWB.instr) == 35) //lw
	{
		newState.MEMWB.writeDataMem = state.dataMem[(state.EXMEM.aluResult)/4];
                //forward checking here
                if((state.EXMEM.writeReg == newState.IDEX.rtReg) && (get_opcode(newState.IDEX.instr) != 35))
                {
                  newState.IDEX.readData2 = newState.MEMWB.writeDataMem;
                }
                if(state.EXMEM.writeReg == newState.IDEX.rsReg)
                {
                  newState.IDEX.readData1 = newState.MEMWB.writeDataMem;
                }

	}
	else if(get_opcode(newState.MEMWB.instr) == 43) //sw
	{
